...
def tracking_codes(state: dict) -> list[str]:
    """平日の追跡対象。層2のうち、翌営業日以降も見続ける必要がある銘柄コード。

    含めるのは2種類:
      - 候補（追跡中）… まだ10日が埋まっておらず、④の合否がこれから決まる
      - 集積の疑い     … 既に成立したもの。**これを外すと、土曜のフル走査で見つけた
        シグナルが月曜の画面から消える**（判定していないだけなのに「該当なし」に見える）。

    逆に「候補（④不成立）」は含めない。結果が確定しており翌日以降に取り直す理由が無く、
    ここを絞らないと平日の取得対象がユニバース全体に近づいて二段構えの意味が無くなる
    （実測: 絞らないと696銘柄、絞ると96銘柄）。
    """
    return sorted({c["code"] for c in state.get("candidates", {}).values()
                   if c.get("layer") == "universe"
                   and str(c.get("kind", "")).startswith(("候補（追跡中", "集積"))})
